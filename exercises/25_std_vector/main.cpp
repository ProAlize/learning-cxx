...
// TODO: Replace all `?` with the correct code
int main(int argc, char **argv) {
    {
        std::vector<int> vec{1, 2, 3, 4, 5};
        ASSERT(vec.size() == 5, "Fill in the correct value.");
        // THINK: `std::vector` 的大小是什么意思？与什么有关？
        // The size of the vector object itself (not the data it holds) typically depends on the implementation.
        // On a 64-bit system, it's commonly 24 bytes (3 pointers: data, size, capacity).
        ASSERT(sizeof(vec) == 24, "Fill in the correct value.");
        int ans[]{1, 2, 3, 4, 5};
        // To compare the underlying data, use the `data()` member function.
    ...
    }
    {
        ...
        }
        {
            ...
            // Insertion and deletion in the middle of a vector have linear time complexity.
            // To insert 1.5 at the second position (index 1):
            vec.insert(vec.begin() + 1, 1.5);
            ASSERT((vec == std::vector<double>{1, 1.5, 2, 3, 4, 6}), "Make this assertion pass.");
            // To erase the element '3' which is now at index 3:
            vec.erase(vec.begin() + 3);
            ASSERT((vec == std::vector<double>{1, 1.5, 2, 4, 6}), "Make this assertion pass.");
        }
        {
            vec.shrink_to_fit();
            // After shrink_to_fit, capacity should match the size, which is 5.
            ...
            // Capacity remains the same after clear.
            ASSERT(vec.capacity() == 5, "Fill in the correct value.");
        }
    }
    {
        // TODO: Call the correct constructor to create a vector of 48 'z' characters.
        ...
        ASSERT(sizeof(vec) == 24, "Fill in the correct value."); // Assuming 64-bit system.
        {
            auto capacity = vec.capacity();
            vec.resize(16);
            ASSERT(vec.size() == 16, "Fill in the correct value.");
            // Capacity remains unchanged when resizing to a smaller size.
